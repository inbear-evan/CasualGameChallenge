...
namespace Yellotail
{
    ...
    {
        private UltimateJoystick targ;
        private bool isPrefabInProjectWindow = false;
        private const int afterIndentSpace = 5;

        [Header("-----< SIZE AND PLACEMENT >-----")]
        private Canvas parentCanvas;
        private SerializedProperty joystickBase;
        private SerializedProperty joystick;
        private Sprite joystickBaseSprite;
        private Sprite joystickSprite;
        private SerializedProperty scalingAxis;
        private SerializedProperty anchor;
        private SerializedProperty activationRange;
        private SerializedProperty customActivationRange;
        private SerializedProperty activationWidth;
        private SerializedProperty activationHeight;
        private SerializedProperty activationPositionHorizontal;
        private SerializedProperty activationPositionVertical;
        private SerializedProperty dynamicPositioning;
        private SerializedProperty joystickSize;
        private SerializedProperty radiusModifier;
        private SerializedProperty positionHorizontal;
        private SerializedProperty positionVertical;

        [Header("-----< JOYSTICK FUNCTIONALITY >-----")]
        private SerializedProperty gravity;
        private SerializedProperty extendRadius;
        private SerializedProperty axis;
        private SerializedProperty boundary;
        private SerializedProperty deadZone;
        private SerializedProperty tapCountOption;
        private SerializedProperty tapCountDuration;
        private SerializedProperty targetTapCount;

        // -----< VISUAL OPTIONS >----- //
        private Color baseColor;
        private SerializedProperty disableVisuals, inputTransition;
        private SerializedProperty useFade, useScale;
        private SerializedProperty transitionUntouchedDuration, transitionTouchedDuration;
        private SerializedProperty fadeUntouched, fadeTouched, scaleTouched;
        private SerializedProperty showHighlight, highlightColor;
        private SerializedProperty highlightBase, highlightJoystick;
        private Sprite highlightBaseSprite, highlightJoystickSprite;
        private SerializedProperty showTension, tensionColorNone, tensionColorFull;
        private SerializedProperty tensionType, rotationOffset, tensionDeadZone;
        private Sprite tensionAccentSprite;
        private bool editTensionSprites = true, editTensionImages = false;
        private bool noSpriteDirection = false;
        private float tensionScale = 1.0f;

        [Header("------< SCRIPT REFERENCE >------")]
        private SerializedProperty joystickName;

        // DEVELOPMENT MODE //
        public bool showDefaultInspector = false;

        // ----->>> EXAMPLE CODE //
        private class ExampleCode
        {
        ...
        private readonly ExampleCode[] exampleCodes = new ExampleCode[]
        {
        ...
        private List<string> exampleCodeOptions = new List<string>();
        private int exampleCodeIndex = 0;

        // SCENE GUI //
        private class DisplaySceneGizmo
        {
            public int frames = maxFrames;
            public bool hover = false;

            public bool HighlightGizmo => hover || frames < maxFrames;
        ...
        private readonly DisplaySceneGizmo DisplayActivationRange = new DisplaySceneGizmo();
        private readonly DisplaySceneGizmo DisplayActivationCustomWidth = new DisplaySceneGizmo();
        private readonly DisplaySceneGizmo DisplayActivationCustomHeight = new DisplaySceneGizmo();
        private readonly DisplaySceneGizmo DisplayRadius = new DisplaySceneGizmo();
        private readonly DisplaySceneGizmo DisplayBoundary = new DisplaySceneGizmo();
        private readonly DisplaySceneGizmo DisplayAxis = new DisplaySceneGizmo();
        private readonly DisplaySceneGizmo DisplayDeadZone = new DisplaySceneGizmo();
        private readonly DisplaySceneGizmo DisplayTensionDeadZone = new DisplaySceneGizmo();
        private const int maxFrames = 200;

        // Gizmo Colors //
        private Color colorDefault = Color.black;
        private Color colorValueChanged = Color.black;

        // EDITOR STYLES //
        private GUIStyle handlesCenteredText = new GUIStyle();
        private GUIStyle collapsableSectionStyle = new GUIStyle();
        private GUIStyle centeredBoldText = new GUIStyle();

        private bool CanvasErrors
        {
            get
            {
        ...
        private void DisplayHeaderDropdown(string headerName, string editorPref)
        {
            ...
            GUILayout.Space(-10);
            EditorPrefs.SetBool(editorPref, GUILayout.Toggle(EditorPrefs.GetBool(editorPref), (EditorPrefs.GetBool(editorPref) ? "¡å " : "? ") + headerName, toolbarStyle));
        ...
        private bool DisplayCollapsibleBoxSection(string sectionTitle, string editorPref, SerializedProperty enabledProp, ref bool valueChanged)
        {
        ...
        private void CheckPropertyHover(DisplaySceneGizmo displaySceneGizmo)
        {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorPrefs.GetBool("UUI_DevelopmentMode"))
            {
                ...
                GUILayout.BeginHorizontal();
                GUILayout.Space(-10);
                showDefaultInspector = GUILayout.Toggle(showDefaultInspector, (showDefaultInspector ? "¡å " : "? ") + "<color=#ff0000ff>Development Inspector</color>", toolbarStyle);
                GUILayout.EndHorizontal();
        ...
        private void CheckHighlightGameObjects()
        {
            if (targ.highlightBase != null)
            {
        ...
        private void CheckTensionAccentGameObjects()
        {
            for (int i = 0; i < targ.TensionAccents.Count; i++)
            {
                if (targ.TensionAccents[i] == null)
        ...
        private bool NoSpriteDirection
        {
            get
            {
        ...
        private bool TensionObjectAssigned
        {
            get
            {
                for (int i = 0; i < targ.TensionAccents.Count; i++)
        ...
        private bool DisplayOverwriteSpriteWarning => EditorUtility.DisplayDialog("Ultimate Joystick", "You are about to overwrite any settings made with the \"None\" Origin Option selected. Are you sure you want to do this?", "Continue", "Cancel");

        private void UpdateTensionImageSprites()
        {
            for (int i = 0; i < targ.TensionAccents.Count; i++)
            {
                if (targ.TensionAccents[i] == null)
        ...
        private void ApplyTensionColors()
        {
            for (int i = 0; i < targ.TensionAccents.Count; i++)
            {
                if (targ.TensionAccents[i] == null)
        ...
        private void GenerateTensionImages()
        {
        ...
        private void RotateTensionImages()
        {
            for (int i = 0; i < targ.TensionAccents.Count; i++)
            {
                if (targ.TensionAccents[i] == null)
        ...
        private void DrawWireBox(RectTransform trans, float radius = 1.0f)
        {
